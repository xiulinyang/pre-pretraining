...
# Code modeled on Heuristic Cores:
...
from l0 import sample_z_from_log_alpha, deterministic_z_from_log_alpha
...
# from transformers.deprecated._archive_maps import GPT_NEOX_PRETRAINED_MODEL_ARCHIVE_LIST  # noqa: F401, E402
...
class PPTNeoXPreTrainedModel(PreTrainedModel):
    """
    ...
    _no_split_modules = ["PPTNeoXLayer"]
...
class PPTNeoXAttention(nn.Module):
    def __init__(self, config):
        ...
        self.read_avg_activation = (
            None  # The avg activation sum across batches (not used in forward)
        )
    ...
    def reset_read_avg_activation(self):
        ...
        self.read_avg_activation = torch.zeros(
            (self.num_attention_heads, self.head_size),
            device=self.log_alpha_heads.device,
        )
        self.bsz_sum = 0
    ...
    def get_avg_activation(self):
        ...
        return (
            None
            if (self.bsz_sum == 0 or not self.should_read_avg_activation)
            else (self.read_avg_activation / self.bsz_sum)
        )
    ...
    def forward(
        ...
        if self.avg_activation is None:
            attn_output = attn_output * z_heads.reshape(1, -1, 1, 1)
        else:
            avg_activation = self.avg_activation.reshape(
                1, self.avg_activation.shape[0], 1, self.avg_activation.shape[1]
            )
            attn_output = (attn_output - avg_activation) * z_heads.reshape(
                1, -1, 1, 1
            ) + avg_activation
...
class GPTNeoXFlashAttention2(PPTNeoXAttention):
    """
...
PPT_NEOX_ATTENTION_CLASSES = {
    "eager": PPTNeoXAttention,
    "flash_attention_2": GPTNeoXFlashAttention2,
}


class PPTNeoXLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        ...
        self.attention = PPT_NEOX_ATTENTION_CLASSES[config._attn_implementation](config)
    ...
    def get_num_alpha_params(self):
        return self.attention.get_num_alpha_params()
    ...
    def forward(
        ...
        attention_layer_outputs, z_sum = (
            attention_layer_outputs[:-1],
            attention_layer_outputs[-1],
        )
...
class PPTNeoXModel(PPTNeoXPreTrainedModel):
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList(
            [PPTNeoXLayer(config) for _ in range(config.num_hidden_layers)]
        )
    ...
    def get_num_alpha_params(self):
        return sum(layer.get_num_alpha_params() for layer in self.layers)
    ...
    def forward(
    ...
    ) -> Union[Tuple, BaseModelOutputWithPastZs]:
        r"""
        ...
        for i, (layer, layer_past) in enumerate(zip(self.layers, past_key_values)):
            ...
            outputs, z_layer_sum = outputs[:-1], outputs[-1]
            z_sum += z_layer_sum
...
@dataclass
class CausalLMOutputWithPastZs(ModelOutput):
...
class PPTNeoXForCausalLM(PPTNeoXPreTrainedModel):
    ...
    def reset_read_avg_activation(self):
        self.gpt_neox.reset_read_avg_activation()

    def get_num_alpha_params(self):
        return self.gpt_neox.get_num_alpha_params()

    def set_threshold_for_deterministic(self, threshold_for_deterministic):
        self.gpt_neox.set_threshold_for_deterministic(threshold_for_deterministic)
    ...
    def forward(
        ...
        return_dict: Optional[bool] = None,
        target_sparsity: Optional[float] = None,
    ) -> Union[Tuple, CausalLMOutputWithPastZs]:
        r"""
        ...
        if isinstance(outputs, BaseModelOutputWithPastZs):
            z_sum = outputs.z_sum
        else:
            z_sum = outputs[-1]
        ...
        if not return_dict:
            output = (
                (lm_logits,)
                + outputs[1:-1]
                + (z_sum, zs_loss, target_sparsity, current_sparsity)
            )
            return ((lm_loss,) + output) if lm_loss is not None else output

        return CausalLMOutputWithPastZs(
            loss=lm_loss,
            logits=lm_logits,
...
if __name__ == "__main__":
    ...
    generator = pipeline(
        "text-generation",
        model=PPTNeoXForCausalLM.from_pretrained("EleutherAI/pythia-160m"),
